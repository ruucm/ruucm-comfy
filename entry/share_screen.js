...
const runtime = {
  ...
  startQueue: async function (id, widget) {
    ...
    if (this.live > 0) {
      console.info("[ruucm 🎾] other node is living: ", id);
      return Promise.resolve(false);
    }
    console.info("[ruucm 🎾] start queue: ", id, this.live);
    ...
    this.queueLoop = async function () {
      ...
      if (q.Running < 1 && q.Pending < 1) {
        ...
        if (currentFrame) {
          ...
          console.info("[ruucm 🎾] current frame APD: ", id, apd, duration);
          if (apd > 1) {
  ...
  stopQueue: async function () {
    console.info("[ruucm 🎾] stop queue: ", this.live);
    this.liveTimer = clearTimeout(this.liveTimer);
    this.live = 0;
    this.liveFrame = "";
    return Promise.resolve(true);
    ...
    return new Promise((resolve) => {
      if (navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
        navigator.mediaDevices
          ...
          .catch((e) => {
            console.info("[ruucm 🎾] get display media fail: ", id, e);
            resolve(false);
        ...
        console.info("[ruucm 🎾] get display media not support: ", id);
  ...
  saveFrame: async function (id, video, update = false) {
    if (!this.screen[id]) {
      console.info("[ruucm 🎾] screen cache invalid: ", id);
      return;
    }
    ...
    try {
      ...
      console.info("[ruucm 🎾] video convert to blob fail: ", id, e);
    }
  },
  ...
  startClipArea: function (id, video) {
    ...
    this.ssa.mouse = function (e) {
      ...
      if (type === "mousedown") {
        // console.info("[ruucm 🎾] mousedown: ", e);
        isDrag = true;
  ...
  updateClipArea: function (nodeId, videoWidth, videoHeight) {
    ...
    if (!node) {
      console.info("[ruucm 🎾] node not found: ", nodeId);
      return;
    }
    ...
    if (!el) {
      console.info("[ruucm 🎾] preview area elment not found: ", nodeId);
      return;
    }

    if (!this.screen[nodeId]) {
      console.info("[ruucm 🎾] screen invalid: ", nodeId);
      return;
    }
    ...
    // update preview
    const { x, y, w, h } = area;
    console.info("[ruucm 🎾] area info: ", area, app.graph);
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name !== "RuucmShareScreen") return;
